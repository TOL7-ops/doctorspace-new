-- Final fix for template_id constraint issue
-- This migration properly handles the notification_templates table structure
...
-- Insert default templates safely (without ON CONFLICT for now)
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.notification_templates (name, title_template, message_template, notification_type, priority, is_active) 
    VALUES ('system_alert', 'System Alert', '{message}', 'system', 'low', true);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.notification_templates (name, title_template, message_template, notification_type, priority, is_active) 
    VALUES ('appointment_confirmed', 'Appointment Confirmed', 'Your appointment with {doctor_name} has been confirmed for {date} at {time}. Please arrive 10 minutes early.', 'appointment', 'medium', true);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.notification_templates (name, title_template, message_template, notification_type, priority, is_active) 
    VALUES ('appointment_reminder', 'Appointment Reminder', 'Reminder: You have an appointment with {doctor_name} on {date} at {time}.', 'appointment', 'medium', true);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.notification_templates (name, title_template, message_template, notification_type, priority, is_active) 
    VALUES ('appointment_cancelled', 'Appointment Cancelled', 'Your appointment with {doctor_name} on {date} at {time} has been cancelled.', 'appointment', 'high', true);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.notification_templates (name, title_template, message_template, notification_type, priority, is_active) 
    VALUES ('appointment_rescheduled', 'Appointment Rescheduled', 'Your appointment with {doctor_name} has been rescheduled to {new_date} at {new_time}.', 'appointment', 'medium', true);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.notification_templates (name, title_template, message_template, notification_type, priority, is_active) 
    VALUES ('new_message', 'New Message', 'You have received a new message from {sender_name}.', 'message', 'low', true);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.notification_templates (name, title_template, message_template, notification_type, priority, is_active) 
    VALUES ('urgent_reminder', 'URGENT: Appointment Reminder', 'URGENT: Your appointment with {doctor_name} is in {hours} hour(s) at {time}. Please confirm your attendance.', 'appointment', 'high', true);
  END IF;
END $$;
