-- Fix for template_id constraint issue - handles actual table structure
-- This migration works with the existing notification_templates table structure

-- First, let's check the actual structure and add missing columns safely

-- Add missing columns to notification_templates if they don't exist
DO $$
BEGIN
  -- Add title_template column if it doesn't exist (for template placeholders)
  IF NOT EXISTS (
  ...
  -- Add message_template column if it doesn't exist (for template placeholders)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'notification_templates' 
...
-- Insert default templates safely using the actual table structure
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.notification_templates (name, title, message, title_template, message_template, notification_type, priority, is_active) 
    VALUES ('system_alert', 'System Alert', 'System notification', 'System Alert', '{message}', 'system', 'low', true);
  END IF;
...
COMMENT ON COLUMN public.notification_templates.message IS 'Default message for this template';
